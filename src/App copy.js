...
class Calculator extends React.Component {
  ...
  recieveButtonClick(e) {
    ...
    if (containsANumber(key)) {
        ...
        if(isMathsFunction(this.state.lastkey)){
    ...
    if (key == "." && !containsDecimal(myinput)) {
      ...
      if (myinput.length > 0&&(!isMathsFunction(this.state.lastkey))) {
        this.setInput(key);
      } else {
        this.setInput("0.");
      }
    ...
    //if its a maths function (exclucing "-"")
    if (isMathsFunction(key)) {
      this.setState({ input: key });
      if (!isMathsFunction(this.state.lastkey) && (this.state.lastkey != "-")) {
        this.setState({ total: this.state.total.concat(this.state.input) });
      }
    this.setLast(key);
    }
  ...
  render() {
    ...
    return (
      <div class="container calculator">
        ...
        <div class="key-pad my-2">{eval("2.+3")}{this.state.lastkey}
